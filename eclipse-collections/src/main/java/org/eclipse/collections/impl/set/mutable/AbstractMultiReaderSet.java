...
abstract class AbstractMultiReaderSet<T>
        extends AbstractMultiReaderMutableCollection<T>
        implements RandomAccess, MultiReaderSet<T>
{
    protected MutableSet<T> delegate;

    protected abstract <K> AbstractMultiReaderSet<K> newSetFrom(MutableSet<K> newDelegate);
    ...
    public MutableSet<T> clone()
    {
        try (LockWrapper wrapper = this.lockWrapper.acquireReadLock())
        {
            return this.newSetFrom(this.delegate.clone());
        }
    }
